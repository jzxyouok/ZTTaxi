...
#import "ZTActivityIndicator.h"

@interface ZTActivityIndicator ()
...
@implementation ZTActivityIndicator {
...
- (void)setMode:(ZTActivityMode)newMode {
...
- (void)setProgress:(float)newProgress {
    ...
    //，
    if (mode == ZTActivityModeDeterminate) {
        if ([NSThread isMainThread]) {
            [self updateProgress];
            [self setNeedsDisplay];
        } else {
...
- (void)updateProgress {
    [(ZTRoundProgressView *)indicator setProgress:progress];
}
...
- (void)updateIndicators {
    ...
    if (mode == ZTActivityModeDeterminate) {
        self.indicator = [[[ZTRoundProgressView alloc] init] autorelease];
    }
    else if (mode == ZTActivityModeCustomView && self.customView != nil){
...
+ (ZTActivityIndicator *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    ZTActivityIndicator *hud = [[ZTActivityIndicator alloc] initWithView:view];
    [view addSubview:hud];
    [hud show:animated];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    ...
    for (UIView *v in [view subviews]) {
        if ([v isKindOfClass:[ZTActivityIndicator class]]) {
            viewToRemove = v;
        }
    }
    if (viewToRemove != nil) {
        ZTActivityIndicator *HUD = (ZTActivityIndicator *)viewToRemove;
...
- (id)initWithView:(UIView *)view {
    ...
    if (!view) {
        [NSException raise:@"ZTActivityViewIsNillException"
                    format:@"The view used in the ZTActivity initializer is nil."];
    }
...
- (id)initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        self.animationType = ZTActivityAnimationFade;
        self.mode = ZTActivityModeCustomView;
        self.labelText = nil;
...
- (void)showUsingAnimation:(BOOL)animated {
    self.alpha = 0.0f;
    if (animated && animationType == ZTActivityAnimationZoom) {
    ...
    if (animated) {
        ...
        self.alpha = 1.0f;
        if (animationType == ZTActivityAnimationZoom) {
...
@implementation ZTRoundProgressView
